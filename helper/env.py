...
def create_runtime_reward(cfg: DictConfig):
    ...
    if "config" in cfg.reward:
        runtime_reward_config = OmegaConf.to_container(cfg.reward["config"], resolve=True)
    else:
...
def create_observer_factory(cfg: DictConfig):
    ...
    if "config" in cfg.feature:
        observer_factory_config = OmegaConf.to_container(cfg.feature["config"], resolve=True)
    else:
...
def make_env(
    ...
    graph_spec = create_graph_spec(**OmegaConf.to_container(cfg.feature.limits, resolve=True))
